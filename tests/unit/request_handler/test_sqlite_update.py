...
class TestHttpSQLiteRequestHandler(unittest.TestCase):
    ...
    def test_config_client_address_key(self):
        ...
        with self._data_store_and_handler(
            ...
            # an IPv6 address to test that such an address is decoded correctly.
            self._call_handle(
                handler,
                '/test/' + system_id,
    ...
    def test_config_request_path(self):
        ...
        with self._data_store_and_handler(config) as (data_store, handler):
            ...
            self._call_handle(
                handler, '/system', expect_status=HTTPStatus.OK)
        ...
        with self._data_store_and_handler(config) as (data_store, handler):
            ...
            self._call_handle(
                handler, '/my/system', expect_can_handle=False)
            self._call_handle(
                handler, '/system', expect_can_handle=False)
            self.assertEqual({'key': 'value'}, data_store.get_data('system'))
            self._call_handle(
    ...
    def test_get_request(self):
        ...
        """
        # We use the delete_data action for this test. We do not set the db_file
        # option because it is set by _data_store_and_handler.
        config = {'action': 'delete_data', 'request_path': '/test'}
        with self._data_store_and_handler(config) as (data_store, handler):
            data_store.set_value('system', 'key', 'value')
            self._call_handle(
                handler,
                '/test/system',
                expect_status=HTTPStatus.METHOD_NOT_ALLOWED, method='GET')
            self.assertEqual({'key': 'value'}, data_store.get_data('system'))
