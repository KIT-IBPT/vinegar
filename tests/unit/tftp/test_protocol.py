...
class TestOpcode(unittest.TestCase):
    ...
    def test_from_bytes(self):
        ...
        self.assertEqual(Opcode.DATA, ErrorCode.from_bytes(b"\x00\x03"))
        self.assertEqual(Opcode.ACK, ErrorCode.from_bytes(b"\x00\x04"))
        self.assertEqual(Opcode.ERROR, ErrorCode.from_bytes(b"\x00\x05"))
        self.assertEqual(Opcode.OPTIONS_ACK, ErrorCode.from_bytes(b"\x00\x06"))
        ...
        self.assertEqual(
            Opcode.READ_REQUEST, Opcode.from_bytes(b"\x00\x05\x00\x01", 2)
        )
...
class TestTransferMode(unittest.TestCase):
    ...
    def test_from_str(self):
        ...
        self.assertEqual(TransferMode.OCTET, TransferMode.from_str("octet"))
        self.assertEqual(TransferMode.OCTET, TransferMode.from_str("OcTET"))
        self.assertEqual(TransferMode.MAIL, TransferMode.from_str("mail"))
        self.assertEqual(TransferMode.MAIL, TransferMode.from_str("MaiL"))
        # An invalid opcode should result in an exception.
        with self.assertRaises(ValueError):
