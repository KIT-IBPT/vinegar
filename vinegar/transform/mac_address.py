...
def normalize(
    ...
    normalized form. This means that each byte of the MAC address is represented
    by exactly two characters (adding a leading "0" if needed). The MAC address
    is also transformed to consistenly use upper or lower case characters and to
    ...
    :param delimiter:
        ``:`` or ``colon`` if the bytes of the MAC address shall be separated by
        a colon and ``-``, ``dash`` or ``minus`` if the bytes of the MAC address
        shall be separated by a minus sign. The default is ``:``.
    ...
    # We convert to the hex numbers to integer and back to a hexadecimal string,
    # using a width of two and upper / lower case depending on the setting.
