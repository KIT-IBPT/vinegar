...
class HttpRequestHandler(abc.ABC):
    ...
    def handle(
        ...
        :param context:
            context object that was returned by ``prepare_context``.
        :return:
            tuple of the HTTP status code, the response headers, and a file-like
            object that provides the data that is transferred to the client. The
            file-like object must provide binary data.
        """
...
class HttpServer:
    ...
    class _DelegatingRequestHandler(http.server.BaseHTTPRequestHandler):
        ...
        This request handler overrides the `log_request` and `log_error` methods
        to direct messages to the module logger. It does not override
        ...
        def log_error(self, format, *args):
            ...
            # it for anything that we want logged, so we simply do nothing here.
            pass
